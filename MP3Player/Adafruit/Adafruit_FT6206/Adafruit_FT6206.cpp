...
void Adafruit_FT6206::readData(uint16_t *x, uint16_t *y) {
    static HANDLE hTOUCH = 0;
    uint32_t arraySize = 16;
    uint8_t i2cdat[16];
    
    // Open the touch interface
    hTOUCH = Open(PJDF_DEVICE_ID_TOUCH_FT6206, 0);
    Ioctl(hTOUCH, PJDF_CTRL_TOUCH_REG, 0, 0);
    Read(hTOUCH, i2cdat, &arraySize);
    Close(hTOUCH);
    touches = i2cdat[0x02];
...
uint8_t Adafruit_FT6206::readRegister8(uint8_t reg) {
    ...
    uint8_t x ;
    // TODO: setup for all cases
    switch(reg) {
        case(FT6206_REG_NUMTOUCHES): 
            // Open the touch interface
            hTOUCH = Open(PJDF_DEVICE_ID_TOUCH_FT6206, 0);
            Ioctl(hTOUCH, PJDF_CTRL_NUM_TOUCHES_REG, 0, 0);
            Read(hTOUCH, i2cdat, &arraySize);
            Close(hTOUCH);
            x = i2cdat[0x00];
            break;
